...
// quest list
const peacekeeperQuests: Quest[] = [
  {
    id: 'supply_plans',
    name: 'Supply Plans',
    description: 'Obtain the Secure Folder 0022 in the bunkhouse on Customs and hand it over to Peacekeeper.',
    objectives: [
      { id: 'find_folder', description: 'Obtain Secure Folder 0022', type: 'checkbox' },
      { id: 'handover', description: 'Hand over Secure Folder 0022', type: 'checkbox' }
    ],
    requiredItems: [
      { id: 'bunkhouse_key', name: 'Portable Bunkhouse Cabin Key', quantity: 1 }
    ],
    leadsTo: ['Fishing Gear'],
    rewards: [
      { type: 'money', name: 'Dollars', amount: 1000 },
      { type: 'xp', name: 'Experience', amount: 3200 }
    ],
    note: 'The folder is in the bunkhouse on Customs.',
    map: 'Customs',
    requiredForKappa: false,
    requiredForNetworkProvider: false,
    itemSpawnLocations: [
      {
        image: '/supply_plans_spawn.jpg',
        description: 'On the table inside the bunkhouse on Customs.'
      }
    ]
  },
  // Add more Peacekeeper quests here...
];

function PeacekeeperQuestList() {
  // ===== State: Quest Completion =====
  const [completed, setCompleted] = React.useState<{ [id: string]: boolean }>(() => {
    const saved = localStorage.getItem('peacekeeperQuestProgress');
  ...
  const [objectiveProgress, setObjectiveProgress] = React.useState<{ [questId: string]: { [objId: string]: number | boolean } }>(() => {
    const saved = localStorage.getItem('peacekeeperObjectiveProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const [optionalItemProgress, setOptionalItemProgress] = React.useState<{ [questId: string]: { [itemId: string]: boolean } }>(() => {
    const saved = localStorage.getItem('peacekeeperOptionalItemProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const [requiredItemProgress, setRequiredItemProgress] = React.useState<{ [questId: string]: { [itemId: string]: boolean } }>(() => {
    const saved = localStorage.getItem('peacekeeperRequiredItemProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const handleRequiredItemCheck = (questId: string, itemId: string) => {
    setRequiredItemProgress(prev => {
      ...
      localStorage.setItem('peacekeeperRequiredItemProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleObjectiveCheck = (questId: string, objId: string) => {
    setObjectiveProgress(prev => {
      ...
      localStorage.setItem('peacekeeperObjectiveProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleOptionalItemCheck = (questId: string, itemId: string) => {
    setOptionalItemProgress(prev => {
      ...
      localStorage.setItem('peacekeeperOptionalItemProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  React.useEffect(() => {
    peacekeeperQuests.forEach(quest => {
      ...
      if (allComplete && !completed[quest.id]) {
        setCompleted(prev => {
          ...
          localStorage.setItem('peacekeeperQuestProgress', JSON.stringify(updated));
          return updated;
        });
  ...
  // ===== RENDER: Quest List =====
  const uniqueMaps = [...new Set(peacekeeperQuests.map(q => q.map).filter(Boolean))];
  const filteredQuests = selectedMap
    ? peacekeeperQuests.filter(q => q.map === selectedMap)
    : peacekeeperQuests;
...
export default PeacekeeperQuestList;