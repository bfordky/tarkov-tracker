...
// Lightkeeper's quest list with objectives
const lightkeeperQuests: Quest[] = [


    {
    id:'order_out',
    name: 'Order From Outside',
    description:'Locate, obtain the electrionic jammer at the electromechanical warehouse on Reserve and stash it at the specified location on woods',
    objectives: [
      {id: 'outside_jam', description:'Locaate and obtain the electronic jammer at the electromechanical warehosue on reserve', type:'checkbox',},
      {id: 'outside_stash', description:'stash the jammer on woods', type:'checkbox',}
    ],
  },
  // Add Lightkeeper quests here...
];

// ===== LightkeeperQuestList Component =====
// Displays Lightkeeper's quests with objectives, checkboxes, counters, FIR tags, required items, optional items (with checkboxes), and visual completion cues
// ===== LightkeeperQuestList Component =====
function LightkeeperQuestList() {
  // ===== State: Quest Completion =====
  const [completed, setCompleted] = React.useState<{ [id: string]: boolean }>(() => {
    const saved = localStorage.getItem('lightkeeperQuestProgress');
  ...
  const [objectiveProgress, setObjectiveProgress] = React.useState<{ [questId: string]: { [objId: string]: number | boolean } }>(() => {
    const saved = localStorage.getItem('lightkeeperObjectiveProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const [optionalItemProgress, setOptionalItemProgress] = React.useState<{ [questId: string]: { [itemId: string]: boolean } }>(() => {
    const saved = localStorage.getItem('lightkeeperOptionalItemProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const [requiredItemProgress, setRequiredItemProgress] = React.useState<{ [questId: string]: { [itemId: string]: boolean } }>(() => {
    const saved = localStorage.getItem('lightkeeperRequiredItemProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const handleRequiredItemCheck = (questId: string, itemId: string) => {
    setRequiredItemProgress(prev => {
      ...
      localStorage.setItem('lightkeeperRequiredItemProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleObjectiveCheck = (questId: string, objId: string) => {
    setObjectiveProgress(prev => {
      ...
      localStorage.setItem('lightkeeperObjectiveProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleOptionalItemCheck = (questId: string, itemId: string) => {
    setOptionalItemProgress(prev => {
      ...
      localStorage.setItem('lightkeeperOptionalItemProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  React.useEffect(() => {
    lightkeeperQuests.forEach(quest => {
      ...
      if (allComplete && !completed[quest.id]) {
        setCompleted(prev => {
          ...
          localStorage.setItem('lightkeeperQuestProgress', JSON.stringify(updated));
          return updated;
        });
  ...
  // ===== RENDER: Quest List =====
  const uniqueMaps = [...new Set(lightkeeperQuests.map(q => q.map).filter(Boolean))];
  const filteredQuests = selectedMap
    ? lightkeeperQuests.filter(q => q.map === selectedMap)
    : lightkeeperQuests;
...
export default LightkeeperQuestList;