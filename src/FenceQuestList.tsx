...
// Fence's quest list with objectives
const fenceQuests: Quest[] = [
    ...
    objectives:[
      {id:'contact_fence', description:'Reach 5.0 standing with fence', type: 'checkbox'}
    ]
  },
  // Add more quests as needed...
];
// ===== FenceQuestList Component =====
// Displays Fence's quests with objectives, checkboxes, counters, FIR tags, required items, optional items (with checkboxes), and visual completion cues
// ===== FenceQuestList Component =====
function FenceQuestList() {
  // ===== State: Quest Completion =====
  const [completed, setCompleted] = React.useState<{ [id: string]: boolean }>(() => {
    const saved = localStorage.getItem('fenceQuestProgress');
  ...
  const [objectiveProgress, setObjectiveProgress] = React.useState<{ [questId: string]: { [objId: string]: number | boolean } }>(() => {
    const saved = localStorage.getItem('fenceObjectiveProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const [optionalItemProgress, setOptionalItemProgress] = React.useState<{ [questId: string]: { [itemId: string]: boolean } }>(() => {
    const saved = localStorage.getItem('fenceOptionalItemProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const [requiredItemProgress, setRequiredItemProgress] = React.useState<{ [questId: string]: { [itemId: string]: boolean } }>(() => {
    const saved = localStorage.getItem('fenceRequiredItemProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const handleRequiredItemCheck = (questId: string, itemId: string) => {
    setRequiredItemProgress(prev => {
      ...
      localStorage.setItem('fenceRequiredItemProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleObjectiveCheck = (questId: string, objId: string) => {
    setObjectiveProgress(prev => {
      ...
      localStorage.setItem('fenceObjectiveProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleOptionalItemCheck = (questId: string, itemId: string) => {
    setOptionalItemProgress(prev => {
      ...
      localStorage.setItem('fenceOptionalItemProgress', JSON.stringify(updated));
      return updated;
    });
  };
    ...
    fenceQuests.forEach(quest => {
      ...
      if (allComplete && !completed[quest.id]) {
        setCompleted(prev => {
          ...
          localStorage.setItem('fenceQuestProgress', JSON.stringify(updated));
          return updated;
        });
  ...
  // ===== RENDER: Quest List =====
  const uniqueMaps = [...new Set(fenceQuests.map(q => q.map).filter(Boolean))];
  const filteredQuests = selectedMap
    ? fenceQuests.filter(q => q.map === selectedMap)
    : fenceQuests;
