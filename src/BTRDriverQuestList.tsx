...
// BTR Driver's quest list with objectives
const btrDriverQuests: Quest[] = [

{
    id:'hot_wheel',
    name: 'Hot Wheels',
    description:'Locate and mark the spare BTR wheels',
    leadsTo: ['Swift Retribution'],
    objectives: [
      {id: 'btr_wheel', description:'Locate and mark the spare BTR wheels on Reserve', type:'checkbox',}
    ],
    requiredItems:[
      {id:'marker', name: 'MS2000 Marker', quantity: 1}
    ],
  },

  // Add BTR Driver quests here...
];

// ===== BTRDriverQuestList Component =====
// Displays BTR Driver's quests with objectives, checkboxes, counters, FIR tags, required items, optional items (with checkboxes), and visual completion cues
// ===== BTRDriverQuestList Component =====
function BTRDriverQuestList() {
  // ===== State: Quest Completion =====
  const [completed, setCompleted] = React.useState<{ [id: string]: boolean }>(() => {
    const saved = localStorage.getItem('btrDriverQuestProgress');
  ...
  const [objectiveProgress, setObjectiveProgress] = React.useState<{ [questId: string]: { [objId: string]: number | boolean } }>(() => {
    const saved = localStorage.getItem('btrDriverObjectiveProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const [optionalItemProgress, setOptionalItemProgress] = React.useState<{ [questId: string]: { [itemId: string]: boolean } }>(() => {
    const saved = localStorage.getItem('btrDriverOptionalItemProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const [requiredItemProgress, setRequiredItemProgress] = React.useState<{ [questId: string]: { [itemId: string]: boolean } }>(() => {
    const saved = localStorage.getItem('btrDriverRequiredItemProgress');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const handleRequiredItemCheck = (questId: string, itemId: string) => {
    setRequiredItemProgress(prev => {
      ...
      localStorage.setItem('btrDriverRequiredItemProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleObjectiveCheck = (questId: string, objId: string) => {
    setObjectiveProgress(prev => {
      ...
      localStorage.setItem('btrDriverObjectiveProgress', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleOptionalItemCheck = (questId: string, itemId: string) => {
    setOptionalItemProgress(prev => {
      ...
      localStorage.setItem('btrDriverOptionalItemProgress', JSON.stringify(updated));
      return updated;
    });
  };
    ...
    btrDriverQuests.forEach(quest => {
      ...
      if (allComplete && !completed[quest.id]) {
        setCompleted(prev => {
          ...
          localStorage.setItem('btrDriverQuestProgress', JSON.stringify(updated));
          return updated;
        });
  ...
  // ===== RENDER: Quest List =====
  const uniqueMaps = [...new Set(btrDriverQuests.map(q => q.map).filter(Boolean))];
  const filteredQuests = selectedMap
    ? btrDriverQuests.filter(q => q.map === selectedMap)
    : btrDriverQuests;
...
export default BTRDriverQuestList;